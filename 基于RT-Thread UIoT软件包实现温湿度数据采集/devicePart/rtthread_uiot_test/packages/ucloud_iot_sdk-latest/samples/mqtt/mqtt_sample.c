 ...
 *
 */
static int _publish_msg(void *client, msg_payload msg_payload)
{
    ...
    int size = HAL_Snprintf(topic_content, sizeof(topic_content), "{\"temp\": \"%d\", \"humi\": \"%d\"}", msg_payload.temp, msg_payload.humi);
    ...
    do {
        ...
        if (sg_sub_packet_id > 0) {
            for(int loop = 0; loop < 10; loop++)
            {
                ...
                payload.temp = temp;
                payload.humi = humi;
                rc = _publish_msg(client, payload);
                if (rc < 0) {
